...
- **距離硬性上限**：發現 Google Places `locationBias` 只是軟性偏好、有時會回傳遠超使用者設定距離的結果（實測案例：設定範圍內卻推薦到 14.3 公里外的店），因此在 `recommend.ts` 加入硬性過濾：推薦結果一律不得超過使用者輸入距離的 2 倍，首輪與次輪放寬都套用同一個上限（以原始輸入距離為準，不隨次輪放寬而跟著放大）。已用 API 直接測試驗證（含次輪放寬情境下候選不足時，正確地只回傳符合上限的餐廳，不會為了湊滿 3 間硬塞超遠結果）。
...
**目前沒有卡關**。LLM 已改用 OpenAI `gpt-4o-mini` 正常運作（原本卡在 Anthropic Console 的付款頁面/信用額度問題，詳見「關鍵設定與上下文」的教訓紀錄），推薦流程已加上距離硬性上限，也已經部署到 Vercel 正式環境。

還沒實測確認的部分（只能在真實瀏覽器操作、curl 測不到，或還沒在正式環境驗證過）：
- 定位權限的瀏覽器彈窗與拒絕後的錯誤畫面。
