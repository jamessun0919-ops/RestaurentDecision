...
**LLM 部分已解除卡關，改用 OpenAI 正常運作**。原本卡在 Anthropic Console 的付款頁面（「Buy USD 5 of credits」按鈕反灰點不下去）；後來確認金鑰其實申請成功、`.env.local` 也填入了 `ANTHROPIC_API_KEY`，但實測時 Anthropic 回覆「credit balance too low」——帳戶餘額是 0，儲值沒有實際入帳，本質上還是同一個付款問題，只是進展到下一關。因為短時間沒解決，決定改申請 `OPENAI_API_KEY`（`gpt-4o-mini`）頂上，已儲值成功並實測 `/api/recommend` 全流程通過（搜尋字串微調、開場白、每間餐廳推薦理由都正常生成）。
...
- 次輪放寬（round 2）流程尚未實測，只測過首輪。

## 下一步行動 (Next Steps)
下次重新開始時，第一件事：回頭做瀏覽器端對端測試（定位權限彈窗、飲食限制硬性過濾、次輪放寬），確認沒問題後討論部署到 Vercel。若 Anthropic Console 的儲值問題之後自行解決，可考慮要不要切回 Claude（非必要，OpenAI 版本已經能動）。
