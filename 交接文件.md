...
- **部署**：預計用 Vercel（尚未實際部署，只在本機開發測試）。
- **生成回覆的 LLM**：目前使用 **Claude API（`claude-haiku-4-5`）**，程式碼在 `src/lib/claude.ts`。中間繞了一圈：一開始因為 Anthropic 申請需先綁信用卡改用 Gemini 免費方案測試 → Gemini 免費額度每日僅 20 次很快用完 → 嘗試用 Google Cloud 試用額度（$9,422）解決，才發現 **Gemini API 走獨立的「Cloud Prepay」計費系統，不會用到 GCP 一般試用額度**，需要另外用信用卡最低儲值 NT$1,000 且不可退款 → 決定改回申請 Anthropic 帳號、改用 Claude Haiku 4.5（依實測 prompt 估算單輪成本約 NT$0.16，比 Gemini 的整筆不可退預付更划算也更符合原始設計）。若之後想換回其他 LLM，只需改 `src/lib/claude.ts` 這個檔案，其他程式不受影響。
- **餐廳資料**：Google Places API (New)，`places:searchText` 端點。
- **金鑰安全**：Google Places、Gemini 金鑰都只存在後端 `.env.local`（已加入 `.gitignore`），前端從未直接呼叫這兩個外部 API，一律透過自家後端路由轉呼叫。
...
src/
  lib/
    ...
    claude.ts        # 呼叫 Claude API：搜尋關鍵字微調、結構化生成推薦文案（zodOutputFormat）
...
### 已實作並端對端測試通過的功能（測試當時用的是 Gemini，邏輯與 Claude 版本相同，見下方瓶頸說明）
...
**卡在 Anthropic Console 的付款頁面**：申請 Claude API 金鑰前要先加入付款方式，「Buy USD 5 of credits」按鈕反灰點不下去，已嘗試無痕視窗、重新整理、檢查必填欄位都沒解決，判斷是 Anthropic 網站端的問題，不是我們程式的問題。

因此：
- `src/lib/claude.ts`（呼叫 Claude Haiku 4.5 的程式碼）已經寫完，`tsc`／`eslint` 都過，但**還沒有真的打過 API**——`.env.local` 裡的 `ANTHROPIC_API_KEY` 還是空的，因為金鑰申請卡在付款頁面。
- 舊的 Gemini 版本邏輯已經被移除（`src/lib/gemini.ts` 已刪除），現在沒有任何可用的 LLM 金鑰，`/api/recommend` 目前無法產生推薦文案。
- 在 Gemini 版本上跑通的端對端測試（Places 搜尋、次輪放寬、照片代理）邏輯沒有變，理論上換成 Claude 也一樣會動，但這是推測，還沒有實測驗證。

還沒有部署到 Vercel，只在 `localhost:3000` 開發模式運行；另外幾個功能因為只能在真實瀏覽器操作、curl 測不到，也還沒有實測確認：
- 定位權限的瀏覽器彈窗與拒絕後的錯誤畫面。
- 素食/過敏限制的實際硬性過濾效果（測試時沒有設定這些條件）。
- 手機實機測試（含 PWA 相關功能，PWA 本身也還沒實作）。

## 下一步行動 (Next Steps)
下次重新開始時，第一件事：**解決 Anthropic Console 的付款卡關問題**（可能需要聯絡 Anthropic 客服）。拿到 `ANTHROPIC_API_KEY` 後貼進 `.env.local`，請我重新測試 `/api/recommend`，確認 Claude Haiku 4.5 版本的搜尋字串微調與結構化推薦文案生成都正常運作。確認沒問題後，再回頭做瀏覽器端對端測試（定位權限、飲食限制過濾），最後討論部署到 Vercel。
...
- LLM 模型：`claude-haiku-4-5`（`src/lib/claude.ts` 裡的 `MODEL` 常數），選這個是因為這個任務（短篇關鍵字/JSON 生成）用最便宜的模型就夠了，實測前估算單輪成本約 NT$0.16。
- Gemini 相關程式碼與金鑰已移除（`GEMINI_API_KEY` 還留在 `.env.local` 裡沒刪，但程式沒有再讀取它，可忽略或之後清掉）。
...
- 安全提醒：`GOOGLE_PLACES_API_KEY`、`ANTHROPIC_API_KEY` 都在 `.env.local`（已 gitignore），任何會把外部 API 網址直接暴露給前端的功能（例如照片），都要透過後端路由代理，不能讓金鑰出現在瀏覽器可見的網址或程式碼裡。
